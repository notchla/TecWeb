...
exports.getStoryNames = async (req, res) => {
  var data = await db.getStories();
  var storynames = [];
  data.forEach(function(el, _) {
    storynames.push(el.title)
  });
  res.send( {storynames : storynames});
};

exports.registerStory = async (req, res) => {
  await db.saveStory(req.body.storyname, req.body.adj, req.body.nodes, req.body.published);
  res.end();
};
...
exports.getActivityForm = async (req, res) => {
  ...
  var formsList = {
    ...
    "open question":
      '<label for="message-text" class="col-form-label"> Enter question </label> ' +
      '<input class="form-control" id="question"></input>' +
